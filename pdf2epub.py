...
class PdfExtractor:
    ...
    def extract(self, start: int = 0, end: int = None, progress_cb=None):
        ...
        for pno in range(start, end):
            if progress_cb:
                progress_cb(5 + int(65 * (pno - start) / total),
                            f"正在解析页面 {pno + 1}/{end}")
            page = self.doc[pno]
            rects = self.page_rects(page)
...
def build_epub(pdf_path: str, out_path: str, start: int = 0, end: int = None,
               progress_cb=None):
    """转换 PDF -> EPUB. progress_cb(percent:int, message:str) 可选进度回调."""
    def report(pct, msg):
        if progress_cb:
            progress_cb(pct, msg)
    ...
    page_blocks = ext.extract(start, end, progress_cb=progress_cb)
    report(72, "正在合并跨页段落…")
    ...
    report(90, f"正在嵌入 {len(ext.images)} 张图片并装订 EPUB…")
